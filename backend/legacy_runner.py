...
class LegacyGuiPipelineRunner:
    ...
    def _build_proxy(self):
        ...
        proxy._filter_frame_quality = MethodType(self._module.SfMApp._filter_frame_quality, proxy)
        proxy._apply_quality_sweep_sample = MethodType(self._module.SfMApp._apply_quality_sweep_sample, proxy)
        ...
        proxy.skip_existing = _Var(self.request.skip_existing)
        proxy.quality_sweep = _Var(self.request.quality_sweep)
        proxy.exclude_blurry = _Var(self.request.exclude_blurry)
        proxy.exclude_black = _Var(self.request.exclude_black)
        proxy.blur_threshold = _Var(self.request.blur_threshold)
        proxy.black_threshold = _Var(self.request.black_threshold)
        proxy.sweep_sample_frames = _Var(self.request.sweep_sample_frames)
